...
#-----------------Data Preprocessing-----------------------#
df = pd.read_csv("/home/application/datasets/dataset_3.csv")
...
def main():
    ...
    valeur3 = [generate_random(7, 100) for _ in range(7)]
    somme_totale3 = sum(valeur3)
    valeurs_normalisee3 = [valeur / somme_totale3 for valeur in valeur3]
    rounds = 0
    W3, b3 = W, b
    #----client socket----#
    client_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    server_conf = ('localhost', 12345)
    client_conf = ('localhost', 12348)
    client_socket.bind(client_conf)
    #----client training----#
    while rounds < 3:
        print("Round : ", rounds)
        W3, b3, dW3, db3 = artificial_neuron(W3, b3, X_train, y_train, learning_rate = 0.01, n_iter = 40)
        params = {'dW' : dW3, 'db': db3}
