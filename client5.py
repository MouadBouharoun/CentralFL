...
#-----------------Data Preprocessing-----------------------#
df = pd.read_csv("/home/application/datasets/dataset_5.csv")
...
def main():
    ...
    valeur5 = [generate_random(7, 100) for _ in range(7)]
    somme_totale5 = sum(valeur5)
    valeurs_normalisee5 = [valeur / somme_totale5 for valeur in valeur5]
    rounds = 0
    W5, b5 = W, b
    #----client socket----#
    client_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    server_conf = ('localhost', 12345)
    client_conf = ('localhost', 12350)
    client_socket.bind(client_conf)
    #----client training----#
    while rounds < 3:
        print("Round : ", rounds)
        W5, b5, dW5, db5 = artificial_neuron(W5, b5, X_train, y_train, learning_rate = 0.01, n_iter = 40)
        params = {'dW' : dW5, 'db': db5}
