...
#-----------------Data Preprocessing-----------------------#
df = pd.read_csv("/home/application/datasets/dataset_4.csv")
...
def main():
    ...
    valeur4 = [generate_random(7, 100) for _ in range(7)]
    somme_totale4 = sum(valeur4)
    valeurs_normalisee3 = [valeur / somme_totale4 for valeur in valeur4]
    rounds = 0
    W4, b4 = W, b
    #----client socket----#
    client_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    server_conf = ('localhost', 12345)
    client_conf = ('localhost', 12349)
    client_socket.bind(client_conf)
    #----client training----#
    while rounds < 3:
        print("Round : ", rounds)
        W4, b4, dW4, db4 = artificial_neuron(W4, b4, X_train, y_train, learning_rate = 0.01, n_iter = 40)
        params = {'dW' : dW4, 'db': db4}
