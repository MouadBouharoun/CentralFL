...
#-----------------Data Preprocessing-----------------------#
df = pd.read_csv("/home/application/datasets/dataset_2.csv")
...
def main():
    ...
    valeur2 = [generate_random(7, 100) for _ in range(7)]
    somme_totale2 = sum(valeur2)
    valeurs_normalisee2 = [valeur / somme_totale2 for valeur in valeur2]
    rounds = 0
    W2, b2 = W, b
    #----client socket----#
    client_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    server_conf = ('localhost', 12345)
    client_conf = ('localhost', 12347)
    client_socket.bind(client_conf)
    #----client training----#
    while rounds < 3:
        print("Round : ", rounds)
        W2, b2, dW2, db2 = artificial_neuron(W2, b2, X_train, y_train, learning_rate = 0.01, n_iter = 40)
        params = {'dW' : dW2, 'db': db2}
